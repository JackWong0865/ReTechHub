...
if(!$product){
    ...
    $_SESSION['publish_error'] =
        "Draft product not found or the product has already been published.";
...
$total_images =
    (int)($image_data['total_images'] ?? 0);

/*require at least one product image*/
if($total_images <= 0){
    $missing_fields[] = "At Least One Product Image";
}
...
if(!empty($missing_fields)){
    ...
    $missing_html =
        "Please complete the following information before publishing:<br><br>";

    foreach($missing_fields as $field){

        $missing_html .=
            "• " . htmlspecialchars($field) . "<br>";
    }

    $_SESSION['publish_error'] =
        $missing_html;

    $_SESSION['publish_product_id'] =
        $product_id;
...
$published =
    mysqli_stmt_execute($publish_stmt);

/*check affected rows*/
$affected_rows =
    mysqli_stmt_affected_rows($publish_stmt);

/*save database error before closing statement*/
$publish_error =
    mysqli_stmt_error($publish_stmt);
